...
def test_round_trip() -> None:
    frame = protocol.hello("tok-123", "pmesh", 3000)
...
def test_truncated_frame_raises_connection_closed() -> None:
    data = protocol.encode_frame(protocol.hello("tok", "pmesh", 3000))
    with pytest.raises(protocol.ConnectionClosed):
...
def test_require_str() -> None:
    frame = protocol.hello("tok", "pmesh", 3000)
    assert protocol.require_str(frame, "subdomain") == "pmesh"
    for bad in (
        {"type": "hello"},
        {"type": "hello", "subdomain": ""},
        {"type": "hello", "subdomain": 7},
    ):
        with pytest.raises(protocol.ProtocolError):
...
def test_redacted_masks_token_and_preserves_original() -> None:
    frame = protocol.hello("secret", "pmesh", 3000)
